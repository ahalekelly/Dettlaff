...
uint32_t revRPM[4] = {
  ...
  50000,
  50000
}; // adjust this to change fps - note that these numbers currently assume you
   // have a 4S battery! will fix soon
...
pins_t pins = pins_v0_4_noid; // select the one that matches your board revision
                              // and pusher type
// Options:
        ...
        switch (pusherType) {
          ...
          case PUSHER_SOLENOID_OPENLOOP:
            if (shotsToFire > 0 && !firing &&
                time_ms >
                  pusherTimer_ms + solenoidRetractTime_ms) { // extend solenoid
              digitalWrite(pins.pusher, HIGH);
              firing = true;
              shotsToFire -= 1;
              pusherTimer_ms = time_ms;
  ...
  if (closedLoopFlywheels) {
    ...
    for (int i = 0; i < numMotors; i++) {
      if (throttleValue[i] == 0) {
        throttleValue[i] = min(maxThrottle,
                               maxThrottle * *targetRPM[i] / batteryADC_mv *
                                 1000 / scaledMotorKv);
      } else {
        throttleValue[i] = max(min(maxThrottle,
                                   maxThrottle * *targetRPM[i] / batteryADC_mv *
                                     1000 / scaledMotorKv),
