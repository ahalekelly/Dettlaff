...
uint32_t revRPM[4] = {
    50000, 50000, 50000,
    50000}; // adjust this to change fps - note that these numbers currently assume you have a 4S battery! will fix soon
...
uint16_t pusherStallTime_ms = 500; // for PUSHER_MOTOR_CLOSEDLOOP, how long do you run the motor without seeing an
                                   // update on the cycle control switch before you decide the motor is stalled?
...
uint32_t firingRPM[4] = {revRPM[0] * 9 / 10,
                         revRPM[1] * 9 / 10, // for closed loop flywheel mode only - not implemented yet
...
void loop()
{
    ...
    case STATE_ACCELERATING:
        if (closedLoopFlywheels) {
            ...
            // pusher delay
            if (motorRPM[0] > firingRPM[0] && (numMotors <= 1 || motorRPM[1] > firingRPM[1]) &&
                (numMotors <= 2 || motorRPM[2] > firingRPM[2]) && (numMotors <= 3 || motorRPM[3] > firingRPM[3])) {
        ...
        if (!revSwitch.isPressed() && shotsToFire == 0 && !firing) {
            ...
            case PUSHER_SOLENOID_OPENLOOP:
                if (shotsToFire > 0 && !firing &&
                    time_ms > pusherTimer_ms + solenoidRetractTime_ms) { // extend solenoid
                    digitalWrite(pins.pusher, HIGH);
                    firing = true;
                    shotsToFire -= 1;
                    pusherTimer_ms = time_ms;
    ...
    if (closedLoopFlywheels) {
        ...
        for (int i = 0; i < numMotors; i++) {
            if (throttleValue[i] == 0) {
                ...
                throttleValue[i] =
                    max(min(maxThrottle, maxThrottle * *targetRPM[i] / batteryADC_mv * 1000 / scaledMotorKv),
