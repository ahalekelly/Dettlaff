...
uint32_t revRPM[4]
    = { 50000, 50000, 50000,
        50000 }; // adjust this to change fps - note that these numbers
                 // currently assume you have a 4S battery! will fix soon
...
uint16_t pusherStallTime_ms
    = 500; // for PUSHER_MOTOR_CLOSEDLOOP, how long do you run the motor
           // without seeing an update on the cycle control switch before you
           // decide the motor is stalled?
...
Servo servo[4];
DShotRMT dshot[4] = { DShotRMT (pins.esc1, RMT_CHANNEL_1),
                      DShotRMT (pins.esc2, RMT_CHANNEL_2),
                      DShotRMT (pins.esc3, RMT_CHANNEL_3),
                      DShotRMT (pins.esc4, RMT_CHANNEL_4) };
...
loop ()
{
      ...
      if (closedLoopFlywheels)
        {
          ...
          // FULLSPEED. in the future add predictive capacity for when they'll
          // be up to speed in the future, taking into account pusher delay
      ...
      else if (shotsToFire > 0 || firing)
        {
          switch (pusherType)
            ...
            case PUSHER_SOLENOID_OPENLOOP:
              if (shotsToFire > 0 && !firing
                  && time_ms > pusherTimer_ms + solenoidRetractTime_ms)
                { // extend solenoid
                  digitalWrite (pins.pusher, HIGH);
                  firing = true;
                  shotsToFire -= 1;
                  ...
                  Serial.println ("solenoid extending");
                       ...
                       && time_ms > pusherTimer_ms + solenoidExtendTime_ms)
                { // retract solenoid
                  digitalWrite (pins.pusher, LOW);
                  firing = false;
                  pusherTimer_ms = time_ms;
      ...
      for (int i = 0; i < numMotors; i++)
        {
          if (throttleValue[i] == 0)
            {
              throttleValue[i] = min (maxThrottle, maxThrottle * *targetRPM[i]
                                                       / batteryADC_mv * 1000
                                                       / scaledMotorKv);
            }
          else
  ...
  if (loopTime_us > targetLoopTime_us)
    ...
    {
      delayMicroseconds (
          max ((long)(0), (long)(targetLoopTime_us - loopTime_us)));
    }
}
